...
Windows ホストへの問い合わせは、ホルダー 1 個について訊いても 20 個について訊いても往復 1 回で済む。だから `end`・`reap`・`status` は自分が扱うホルダー全部をまとめて訊く。測ったマシンでは、ホルダー 4 個を解放する `reap` が 3.7 秒から 1.3 秒に、同じ 4 個に対する `status` が 1.9 秒から 0.4 秒になった。
...
ホルダーの解放は 1 分待ってから行う。ターンが終わることと、セッションが終わることは同じではない。サブエージェントを頼めばターンはその実行中に終わり、Claude Code はサブエージェントが終わったときにセッションへ戻ってくる。実行中はサブエージェント用のホルダーが覆っているが、それは `SubagentStop` と一緒に消える。そして `SubagentStop` はセッションが起こされるより前に着く。この受け渡しの隙間でマシンが眠り込むと、起こしに来た合図を受け取れない。

待ってから解放すれば、セッションが何を待っていたかを知らなくても隙間が埋まる。知らなくて済むことが効くのは、wakeguard から見えない起こされ方のほう。API エラーで終わったターンの `StopFailure` には何が動いているかが渡らないし、`/loop` や `ScheduleWakeup` の予約は cron 式で届くので発火時刻が読めない。待っている間に始まったターンはそのホルダーを引き継ぎ、遅れて解放しに来た側がそれを殺さないのは、上に書いた `prompt_id` の照合があるため。
...
| `WAKEGUARD_GRACE_SECONDS` | ホルダーを解放するまで待つ秒数。待っていた作業に起こされたセッションが、マシンが起きたままの状態を見つけられるようにするため。`0` にすると即座に解放する。フック自身の timeout (このプラグインが同梱するフックでは 5 分) より短くしておくこと。超えると解放が実行される前に kill され、`SessionEnd` に委ねられる | `60` |
...
| 終わり方 | 解除する仕組み |
|---|---|
| ターンが正常終了するか API エラーで終わる | `Stop` / `StopFailure` フック。`WAKEGUARD_GRACE_SECONDS` だけ待ってから |
...
**把握しておくべき穴が 3 つある。** どれも上の待ち時間より長く続くので、一瞬の隙間ではなく穴になる。 Esc でターンを中断しても `Stop` は発火しないので、ホルダーは次のターンが終わるかセッションが終わるまで残る。それまで PC は自分からは眠らない。サブエージェントごと中断した場合に `SubagentStop` が届くかは未確認で、届かなければサブエージェント用のホルダーも同じだけ残る。3 つめは、`Stop` フック (wakeguard のものとは限らない) が block を返してターンをモデルに差し戻す経路。継続にあたって新しいプロンプトは投入されないので、wakeguard はターンが終わったものとしてホルダーを解除し、モデルはその後も動き続ける。
...
**`Bash` のバックグラウンド実行**。`Stop` は何が動いているかを伝えてくるので、バックグラウンドのシェルが終わるまでマシンを起こしたままにすることはできる。ただし終わらないシェルなら、ホルダー自身の期限まで起こしたままになる。それは、このプラグインが避けるために存在している「抑制を解除できないまま残す」状態そのもの。上の待ち時間の内に終わるシェルは、他と同じように覆われる。それより長く走るものは覆わない。
