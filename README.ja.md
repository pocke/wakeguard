...
待ってから解放すれば、セッションが何を待っていたかを知らなくても隙間が埋まる。知らなくて済むことが効くのは、wakeguard からは存在すら見えない起こされ方のほう。API エラーで終わったターンの `StopFailure` には何が動いているかが渡らないし、`/loop` や `ScheduleWakeup` の予約は cron 式で届くので発火時刻が読めない。どちらも、待っている間に起きれば覆えるし、過ぎてから起きれば覆えない。待っている間にホルダーを引き継ぐもの — 起こされて始まったターンでも、2 つめのサブエージェントでも — は pidfile を書き直す。書き直されているのを見つけた解放は手を出さない。これが、`prompt_id` の届かないところで遅れた解放を安全にしている。サブエージェントの `stop` はそもそもターンを名乗らない。
