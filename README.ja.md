...
- **Claude Code 2.1.72 以上**。フックをターンの待ち時間から外すために要る。バックグラウンドで走らせたフックに stdin を渡すようになったのが 2.1.72 で、wakeguard が使うセッション ID はそこから届く。これより古いバージョンはこの指定を知らないので、従来どおりフックの終了を待つ。動作は変わらず、プロンプトを送ったときに待ちが入るだけ
...
`end` を除く全部をバックグラウンドで走らせるので、Claude Code はフックの終了を待たない。Windows ホストにホルダーを置くには `powershell.exe` との往復が要り、測ったマシンでは 1 回 0.3 秒かかる。1 ターンにつき 2 回これを払う理由はない。
...
pidfile は `${XDG_STATE_HOME:-~/.local/state}/wakeguard/sessions/` に置く。ターン用が `<session_id>.pid`、サブエージェント用が `<session_id>.<agent_id>.pid`。ロックは隣の `../locks/` に、pidfile 1 つにつきディレクトリ 1 つを作る。
...
| バックグラウンドのフックが走っている最中にセッションが閉じる | Claude Code がそのフックを kill する。結果は 1 つ上の行と同じ。そもそもこの隙間が生まれるのは interop の往復で待っている間なので、起きるのは Windows と WSL2、つまり片付けが動く環境に限られる |
