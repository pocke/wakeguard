...
ターン用とサブエージェント用のホルダーは、フックが解放を告げてから 1 分待って解放する。ターンが終わることと、セッションが終わることは同じではない。サブエージェントを頼めばターンはその実行中に終わり、Claude Code はサブエージェントが終わったときにセッションへ戻ってくる。実行中はサブエージェント用のホルダーが覆っているが、それは `SubagentStop` と一緒に消える。そして `SubagentStop` はセッションが起こされるより前に着く。この受け渡しの隙間でマシンが眠り込むと、起こしに来た合図を受け取れない。

待ってから解放すれば、セッションが何を待っていたかを知らなくても隙間が埋まる。知らなくて済むことが効くのは、wakeguard からは存在すら見えない起こされ方のほう。API エラーで終わったターンの `StopFailure` には何が動いているかが渡らないし、`/loop` や `ScheduleWakeup` の予約は cron 式で届くので発火時刻が読めない。どちらも、待っている間に起きれば覆えるし、過ぎてから起きれば覆えない。待っている間に始まったターンはそのホルダーを引き継ぎ、遅れて解放しに来た側がそれを殺さないのは、上に書いた `prompt_id` の照合があるため。
...
| `WAKEGUARD_GRACE_SECONDS` | ホルダーを解放するまで待つ秒数。待っていた作業に起こされたセッションが動き出すまで、マシンを起こしたままにしておくため。`0` にすると即座に解放する。上限は 240 秒で、超える値を書くと既定値に戻す。フック自身の timeout (同梱のフックでは 5 分) の内側に収めるため | `60` |
...
抑制はターンの終わりから `WAKEGUARD_GRACE_SECONDS` だけ長く続く。ターンが終わって 1 分以内に見えるホルダーは、取り残されたものではなく、これから解放されるもの。
...
| サブエージェントが終わる | `SubagentStop` フック。`WAKEGUARD_GRACE_SECONDS` だけ待ってから。マシンが眠り込んでいたのはこの解放の後 |
...
**`Bash` のバックグラウンド実行**。`Stop` は何が動いているかを伝えてくるので、バックグラウンドのシェルが終わるまでマシンを起こしたままにすることはできる。ただし終わらないシェルなら、ホルダー自身の期限まで起こしたままになる。それこそが、このプラグインが避けたい「抑制を解除できないまま残す」状態。上の待ち時間の内に終わるシェルは他と同じように覆うが、それより長く走るものは覆わない。
